...
// the physics engine can calculate the collision for a sphere much faster than for any other object
const cubeShape = new CANNON.Sphere(0.5); // new CANNON.Box(new CANNON.Vec3(0.5, 0.5, 0.5)) // often in 3D games the shape of the physics object does not match the shape of the 3D object
...
scene.add(planeMesh);
// the following plane stops objects from falling when they touch it; this plane is infinitely large
...
function animate() {
  ...
  delta = Math.min(clock.getDelta(), 0.1); // use clock.getDelta() only if its smaller than 0.1
  world.step(delta);
