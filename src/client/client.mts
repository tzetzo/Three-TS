...
// this plane stops the bodies from falling down
...
function splitObject(userData: any, contact: CANNON.ContactEquation) {
  ...
  if (meshes[contactId]) {
    ...
    shards.forEach((d: THREE.Object3D) => {
      ...
      const shape = geometryToShape((d as THREE.Mesh).geometry);
...
function geometryToShape(geometry: THREE.BufferGeometry) {
...
function animate() {
  ...
  // adjust the 3D objects position & rotation to the physical objects position & rotation
