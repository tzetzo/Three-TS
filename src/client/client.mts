  ...
  function (gltf) {
    gltf.scene.traverse(function (child) {
      if ((child as THREE.Mesh).isMesh) {
        ...
        if (m.userData.name !== "Plane") sceneMeshes.push(m); // controlling which objects to intersect with the raycaster
      }
...
function onMouseMove(event: MouseEvent) {
  ...
  const intersects = raycaster.intersectObjects(sceneMeshes, false); // false means to not intersect child objects!!!
